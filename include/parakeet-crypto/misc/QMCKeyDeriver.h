...
class QMCKeyDeriver {
 public:
  ...
  virtual bool FromEKey(std::vector<uint8_t>& out, std::span<const uint8_t> ekey) const = 0;
  virtual bool ToEKey(std::vector<uint8_t>& out, std::span<const uint8_t> key) const = 0;
