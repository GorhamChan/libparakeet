...
std::array<uint8_t, 256> kExpectedKey = {
...
TEST(KeyCrypto, DecryptBasicKeyDecryption)
{
    ...
    auto key = key_crypto->Decrypt(reinterpret_cast<const uint8_t *>(kEncryptedKeyV1.c_str()), kEncryptedKeyV1.size());
    ASSERT_NE(key.size(), 0);
