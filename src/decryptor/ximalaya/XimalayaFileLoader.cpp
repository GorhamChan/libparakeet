...
class XimalayaFileLoaderImpl : public StreamDecryptor
{
  ...
  public:
    XimalayaFileLoaderImpl(std::span<const uint8_t, kX3MContentKeySize> content_key,
                           std::span<const uint16_t, kXmlyScrambleTableSize> scramble_table, const char *subtype)
    {
...
void UpgradeX2MKey(std::span<uint8_t, kX3MContentKeySize> x3m_key, std::span<const uint8_t, kX2MContentKeySize> x2m_key)
{
...
std::unique_ptr<StreamDecryptor> CreateXimalayaDecryptor(
    std::span<const uint8_t> content_key, std::span<const uint16_t, kXmlyScrambleTableSize> scramble_table)
{
...
std::unique_ptr<StreamDecryptor> CreateXimalayaDecryptor(std::span<const uint8_t> content_key, double init_value,
                                                         double step_value)
{
