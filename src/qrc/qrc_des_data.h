...
namespace parakeet_crypto::qrc::data
{

constexpr std::array<uint64_t, 64> des_u64_shift_table = ([] {
    std::array<uint64_t, 64> table{};
    for (uint32_t i = 0; i < 32; i++)
    {
        table[i] = uint64_t{1} << (31 - i);
        table[i + 32] = uint64_t{1} << (31 - i + 32);
    }
    return table;
})();

constexpr std::array<std::array<uint8_t, 64>, 8> g_sboxes = {{
    ...
    {15, 3,  1,  13, 8, 4,  14, 7, 6,  15, 11, 2,  3,  8, 4, 15, 9, 12, 7, 0, 2,  1, 13, 10, 12, 6, 0, 9, 5, 11, 10, 5,
    ...
    {7, 13, 13, 8, 14, 11, 3, 5,  0,  6, 6,  15, 9, 0,  10, 3, 1, 4, 2,  7,  8,  2,
     5, 12, 11, 1, 12, 10, 4, 14, 15, 9, 10, 3,  6, 15, 9,  0, 0, 6, 12, 10, 11, 10,
...
constexpr std::array<uint8_t, 64> kIpTable = {58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4,
                                              62, 54, 46, 38, 30, 22, 14, 6, 64, 56, 48, 40, 32, 24, 16, 8,
                                              57, 49, 41, 33, 25, 17, 9,  1, 59, 51, 43, 35, 27, 19, 11, 3,
                                              61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7};

constexpr std::array<uint8_t, 64> kIpInvTable = {40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31,
                                                 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29,
                                                 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27,
                                                 34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9,  49, 17, 57, 25};
constexpr std::array<uint8_t, 28> key_perm_c = {57, 49, 41, 33, 25, 17, 9,  1,  58, 50, 42, 34, 26, 18,
                                                10, 2,  59, 51, 43, 35, 27, 19, 11, 3,  60, 52, 44, 36};
constexpr std::array<uint8_t, 28> key_perm_d = {63, 55, 47, 39, 31, 23, 15, 7,  62, 54, 46, 38, 30, 22,
                                                14, 6,  61, 53, 45, 37, 29, 21, 13, 5,  28, 20, 12, 4};

constexpr std::array<uint8_t, 16> key_rnd_shift = {
    1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1,
};

constexpr std::array<uint8_t, 24> kKeyCompressionTablePart1 = {14, 17, 11, 24, 1,  5, 3,  28, 15, 6,  21, 10,
                                                               23, 19, 12, 4,  26, 8, 16, 7,  27, 20, 13, 2};
constexpr std::array<uint8_t, 24> kKeyCompressionTablePart2 = {
